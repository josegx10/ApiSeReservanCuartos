    ...
    public function Ver($num, $id){
        if($num == 1){ //Image Lugar Individual
            ...
            foreach ($query as $valor) {
                $query =  Imagenes::
                where('id_lugar',$valor->id)->first();
                array_push($array, $query);
            }
            return response()->json($array);
        }else if( $num == 2){ //Image Lugar ALL
            $query =  Imagenes::
            where('id_lugar',$id)->get();
            return response()->json($query);
        } else if($num == 3){ //Todas las categorias del lugar
            $query =  Lugar_categorias::
            where('ID_lugar',$id)->get();
            $array = array();
            foreach ($query as $valor){
                $json = Categorias::
                where('id',$valor->ID_categorias)->first();
